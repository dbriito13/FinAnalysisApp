...
class TickerFetcher:
    ...
    def fetch_prices(self, ticker):
        ...
        url += str(ticker)
        url += "?period1=1030060800&period2=1684368000&interval=1d&events=history&"
        "includeAdjustedClose=true"
        r = requests.get(url, headers=headers)
        print(f"{r.text}", file=sys.stderr, flush=True)
    ...
    def get_ticker_info(self, ticker):
        ...
        prev_close = df["Adj Close"][-2]
        daily_change = (df["Adj Close"][-1] -
                        df["Adj Close"][-2])/df["Adj Close"][-2] * 100.0
        
        eps_ttm, pe_ratio = self.fetch_metrics(ticker)
        
        return prev_close, daily_change, eps_ttm, pe_ratio
